...
选填车辆匀速通过的预计车速后，还可获得超载恒定载荷区段与累计超载毫秒数，判断超载暴露多久。
...
位置重复只返回错误信封；以及提供预计车速时的持续超载区段载荷与向上取整时长、
不同恒定载荷区段不合并、同位移瞬时超载仅影响原峰值且累计为零、
省略车速的既有响应字节不变等），
全部通过则退出码 0。
...
| `speed_mm_per_s` | 整数 | **选填**：车辆匀速通过桥面的预计车速，毫米每秒；提供时在峰值结论之外返回超载区段与累计超载时长 | 1～50000；缺省或显式 `null` 时不执行区段分析 |

任一约束不满足（含字段缺失、位置重复或未递增、数值越界）都**统一返回 HTTP 422**，
响应体只有错误信封 `{"error": ...}`，**不会生成任何分析结果**。
`speed_mm_per_s` 字段类型错误（小数、字符串、布尔、数组、对象等）、为零或
超出 1～50000 范围时同样 **422 且不附带任何分析结果**；缺省或 `null` 时仍执行
当前峰值分析，**成功响应与引入车速前逐字节一致**（不出现区段字段）。
...
7. **超载区段（仅在提供 `speed_mm_per_s` 时计算）**：相邻事件位移之间没有
   轴进入或离开，桥面载荷恒定；领域层复用上面的轴进入、离开事件生成这些
   相邻位移间的**恒定载荷区段**。只返回**位移长度大于零且载荷严格超过
   核定载荷**的区段，每项给出起始位移、结束位移、该段恒定载荷，以及按
   **“位移差 × 1000 ÷ 车速”向上取整**计算的持续毫秒数；`total_overload_duration_ms`
   为全部区段毫秒数之和。
8. **相邻且载荷相同的区段先合并**（合并后按总位移差一次向上取整）；
   同一位移处“进入先于离开”产生的**瞬时峰值位移长度为零**，它**继续参与
   第 4 条的原峰值裁决**（可触发拦停），但**不进入区段列表、不计入累计时长**。
9. 区段的起止位移与毫秒数同样按**任意精度整数**计算：极大轴位置使离开位移
   超出 int64、极大位移配合 1 毫米每秒的最低车速使毫秒数超出 int64 时仍精确。
...
请求携带合法 `speed_mm_per_s` 时，上面五个原峰值字段**原样保留**，响应末尾
**追加** `overload_segments` 与 `total_overload_duration_ms`：

```bash
# 两轴各 7000kg、间距 1000mm、桥长 3000mm、核定 10000kg、车速 3000mm/s：
# 两轴同时落桥的超载区间为位移 [1000,3000)，恒定载荷 14000kg；
# 毫秒数 2000×1000÷3000 = 666.66… 向上取整为 667。
curl -s -X POST localhost:8080/api/v1/bridge-window \
  -H 'Content-Type: application/json' \
  -d '{"axle_positions_mm":[0,1000],"axle_loads_kg":[7000,7000],"bridge_length_mm":3000,"approved_load_kg":10000,"speed_mm_per_s":3000}'
```
...
```json
{
  ...
  "overload_segments": [
    {
      "start_displacement_mm": 1000,
      "end_displacement_mm": 3000,
      "load_kg": 14000,
      "duration_ms": 667
    }
  ],
  "total_overload_duration_ms": 667
}
```

| 字段 | 含义 |
| --- | --- |
| `overload_segments` | 车辆匀速通过期间桥面载荷**严格超过核定载荷**的恒定载荷区段，按位移升序；仅含位移长度大于零的区段，相邻且载荷相同的已先合并；无超载时为 `[]` |
| `overload_segments[].start_displacement_mm` / `end_displacement_mm` | 区段的起止车辆位移（半开区间 `[start, end)`，毫米），均为精确 JSON 数字 |
| `overload_segments[].load_kg` | 该区段内恒定的桥面载荷合计，千克（精确 JSON 数字） |
| `overload_segments[].duration_ms` | 该区段持续毫秒数：`(end − start) × 1000 ÷ speed_mm_per_s` **向上取整** |
| `total_overload_duration_ms` | 全部区段 `duration_ms` 之和；无超载区段时为 `0`（精确 JSON 数字） |

同一位移处进入先于离开形成的**瞬时超载**（如边界恰好容纳瞬间三轴全落桥）仍
计入 `max_load_kg` 并可使 `conclusion` 为 `拦停`，但因位移长度为零，
`overload_segments` 为 `[]`、`total_overload_duration_ms` 为 `0`。
未提供 `speed_mm_per_s`（或缺省/`null`）时这两个字段**整体省略**，响应与
引入车速能力前**逐字节一致**。
...
`speed_mm_per_s` 为零、越界或类型错误时例如：

```json
{ "error": "预计车速 0 超出允许范围 1-50000 毫米每秒" }
```
...
| 预计车速 `speed_mm_per_s` | 选填整数，范围 1～50000 毫米每秒；缺省或 `null` 时只做原峰值分析，成功响应与引入车速前逐字节一致；类型错误、为零或越界 422 且不附带分析结果 |
| 超载区段 | 仅提供车速时返回：相邻事件位移间载荷恒定，只保留位移长度大于零且载荷严格超过核定载荷的区段，相邻且载荷相同先合并；每项给起止位移、恒定载荷、`位移差×1000÷车速` 向上取整的毫秒数，并给累计毫秒数 |
| 瞬时超载 | 同位移处进入先于离开产生的零长度瞬时峰值继续参与原峰值裁决（可拦停），但不进入区段、不计累计时长 |
